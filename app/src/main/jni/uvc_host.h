#ifndef _UVCHOST_H_
#define _UVCHOST_H_

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <fcntl.h>
#include <sys/mman.h>
#include <time.h>
#include <sys/ioctl.h>
#include <sys/time.h>
#include <linux/videodev2.h>
#include "videodev2_complmt.h" // for Android API 19

#define NUMBER_OF_BUFFERS 4
#define FPS_SHOW_INTERVAL 3

/* For JPEG File */
const static unsigned char dht_data[] = {
    
};

typedef enum {
    UVC_DEVICE_STAGE_IDEL,
    UVC_DEVICE_STAGE_INIT,
    UVC_DEVICE_STAGE_OPEN,
    UVC_DEVICE_STAGE_GET_FORMAT_NUMBER,
    UVC_DEVICE_STAGE_STORE_FORMAT,
    UVC_DEVICE_STAGE_SET_FORMAT,
    UVC_DEVICE_STAGE_SET_FPS,
    UVC_DEVICE_STAGE_REQ_BUF,
    UVC_DEVICE_STAGE_MAP_BUF,
    UVC_DEVICE_STAGE_JPEG_BUF, // malloc buf for JPEG streaming data
    UVC_DEVICE_STAGE_START_CAPTURE
} UVC_DEVICE_STAGE;

typedef struct _UvcDeviceFormat{
    int                     index;
    struct v4l2_frmivalenum fival;
} UvcDeviceFormat;

/* Structure for UVC Device */
typedef struct _UvcDeviceInfo{
    char                       uvc_dev_path[16];
    int                        uvc_dev_fd;
    int                        width;
    int                        height;
    struct v4l2_capability     cap;
    struct v4l2_fmtdesc        fmt;
    struct v4l2_frmsizeenum    fsize;
    struct v4l2_frmivalenum    fival;
    int                        format_num;
    UvcDeviceFormat            *sup_format;
    struct v4l2_format         set_format;
    struct v4l2_streamparm     *set_fps;
    struct v4l2_requestbuffers req_buf;
    struct v4l2_buffer         buf;
    UVC_DEVICE_STAGE           stage;
    char                       *jpeg_buf;
    int                        jpeg_buf_used_size;
    void                       *memory[NUMBER_OF_BUFFERS]; // for map
    struct timeval             tv;
    unsigned int               fps_count;
    unsigned long              fps_count_start_time_us;
    unsigned long              fps_count_cur_time_us;
    int                        fps_show_interval;
} UvcDeviceInfo;

int print_v4l2_cap(struct v4l2_capability *cap);
int is_huffman(unsigned char *buf);
void write_jpg_file(unsigned char *buf, int size, char *file_name);
int write_jpg_memory(unsigned char *buf, int size, char *memory_ptr);

int open_uvc_device(UvcDeviceInfo *uvc_device_info);
void close_uvc_device(UvcDeviceInfo *uvc_device_info);
void init_uvc_device_info(UvcDeviceInfo *uvc_device_info, char *dev_name, int dev_name_length, int width, int height);
int count_uvc_format_number(UvcDeviceInfo *uvc_device_info);
int store_uvc_format(UvcDeviceInfo *uvc_device_info);
void print_support_format(UvcDeviceInfo *uvc_device_info);
int set_uvc_format(UvcDeviceInfo *uvc_device_info, int width, int height, int format);
int set_uvc_fps(UvcDeviceInfo *uvc_device_info, int frame_interval_numerator, int frame_interval_denominator);
int create_uvc_buf(UvcDeviceInfo *uvc_device_info);
int start_uvc_capture(UvcDeviceInfo *uvc_device_info);
int stop_uvc_capture(UvcDeviceInfo *uvc_device_info);
int get_vuc_frame(UvcDeviceInfo *uvc_device_info);

#endif
